## [브로셔](https://www.notion.so/teamsparta/11-c22c35d1b86f463e994be20ba5bee39f)
...
### 회원 기능
> 회원 생성부터 권한에 따른 서비스 기능

[모든 회원]
- [X] 회원가입
  - 프로필 이미지 업로드 지원 (선택사항)
  - 회원가입 시 4가지 권한 중 하나 선택 (CUSTOMER, OWNER, MANAGER, MASTER)
  - 유효성 검사를 통한 데이터 검증
- [X] 회원 로그인
  - 회원 인증 기능
  - JWT 토큰 방식으로 인증 처리
- [X] 회원 정보 수정
  ...
  - 기본 정보 수정 가능
- [X] 회원 탈퇴
  - 모든 권한의 회원이 자신의 계정 탈퇴 가능
  - 탈퇴 시 관련 데이터 처리
    
[ADMIN] (**MANAGER, MASTER 권한을 가진 회원만 조회 가능)
- [X] 회원 전체 조회
  - 페이지네이션 지원
  - 정렬 기능 지원 (기본값: 생성일자 기준 내림차순)
  - 전체 회원 목록 조회 가능
- [X] 회원 단일 조회
  - 상세 프로필 정보 제공
  - 회원 정보에 직접 접근할 수 있음
- [X] 회원 탈퇴 처리
  - MANAGER, MASTER 권한을 가진 회원만 조회 가능
  - 회원의 정보를 운영 정책에 따라 탈퇴 처리할 수 있음
  - 탈퇴 시 관련 데이터 처리
